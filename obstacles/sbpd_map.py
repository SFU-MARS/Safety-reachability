...
class SBPDMap(ObstacleMap):
    ...
    def __init__(self, params):
        ...
        self._initialize_fmm_map() # Intialize fmm map with obstacle as goal
    ...
    def _initialize_occupancy_grid_for_map(self):
        ...
        """
        # map_path_name = '/home/anjianl/Desktop/project/WayPtNav/data/obstacle_map/area1_traverse.npy'

        resolution, traversible = self._r.get_config()
        #traversible=traversible[140:200,340:400]
        # np.save(map_path_name, traversible)

        # if os.path.isfile(map_path_name):
        #     resolution = 5
        #     traversible = np.load(map_path_name)
        # else:
        #     raise Exception('No map traversible!')

        self.p.dx = resolution / 100.  # To convert to metres.

        # Reverse the shape as indexing into the traversible is reversed ([y, x] indexing)
    ...
    def _initialize_fmm_map(self):
        ...
        """
        p = self.p
        occupied_xy_m2 = np.array(np.where(self.occupancy_grid_map)).T #nonzero
    ...
    def get_observation(self, config=None, pos_n3=None, **kwargs):
        ...
        if config is not None:
            pos_n3 = config.position_and_heading_nk3()[:, 0].numpy() #impo
