...
import copy
from reachability.Grid import Grid
...
class ReachabilityMap(object):
    ...
    def __init__(self,
                 goal_grid_2d,
                 obstacle_grid_2d, map_origin_2d, map_bdry_2d, dx, start_pos_2d,
                 goal_pos_2d,
                 params):
        """
    ...
    def _reset_variables(self, update_avoid_4d_whole=False):
        """
        ...
        if update_avoid_4d_whole:
            ...
            # Note: we won't get in here anyway,
            # else:
    ...
    def _create_avoid_4d_map_new(self, brt_4d_whole):
        # Configure VoxelMap4d params
        map_size_4d = brt_4d_whole.shape
        map_origin_4d = self.grid.min
        dtheta = self.grid.dx[2]
        dv = self.grid.dx[3]
        dx = self.grid.dx[0]
        dy = self.grid.dx[1]

        # Create a VoxelMap4d
        self.avoid_4d_map = VoxelMap4d(dx=dx, dy=dy, dtheta=dtheta, dv=dv,
                                       origin_4=map_origin_4d,
                                       map_size_4=np.asarray(map_size_4d))

        self.avoid_4d_map.voxel_function_4d = tf.convert_to_tensor(brt_4d_whole, dtype=tf.float32)

    def _reset_avoid_4d_map_from_whole(self, brt_4d_whole):
        """
        ...
        clipped_avoid_4d_ttr = brt_4d_whole[lower_x:(upper_x + 1), lower_y:(upper_y + 1), :, :]
