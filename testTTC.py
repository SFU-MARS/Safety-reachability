...
# data=np.load("ttr_avoid_map_4d_whole_area3_no_dist.npy")
# vind=26#27=speed3, 10=speed 0
vind=8
# v_range=np.arange(-1.5,3.5, 1/6)
v_range=np.arange(-.1,.7, 9)
# print(v_range[vind,])
TTC=data[:,:,vind,:]
...
# plt.imshow(image)
# plt.show()
pos0=ax[0,0].matshow(TTC1)
# ax[0,0].set_title('theta=-180')
ax[0,0].set_title('theta=90')#(-180-90)%360
...
plt.suptitle('v=vhigh=3, dxy=0.05 , dtheta=0.15 , uw=0.4, ua=1.1', x=0.1, y=.95, horizontalalignment='left', verticalalignment='bottom', fontsize = 15)
