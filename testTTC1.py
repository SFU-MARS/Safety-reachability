...
data=np.load("/local-scratch/tara/project/WayPtNav-reachability/reachability/data_tmp/avoid_map_4d/v1/TTR_grid_All_Anjians_test.npy")
# data=np.load("/local-scratch/tara/project/WayPtNav-reachability/reachability/data_tmp/avoid_map_4d/v1/ttr_avoid_map_4d_whole_area3_no_dist.npy")
vind=23#27=speed3, 10=speed 0
# vind=2
# v_range=np.arange(-1.5,3.5, 1/6)
v_range=np.arange(-.1-0.3,.7+0.3, (1+0.4)/30)
TTC=data[:,:,vind,:]
...
# rect = matplotlib.patches.Rectangle((x1,y1),x2-x1,y2-y1, edgecolor='r', facecolor="none")
# TTC1=TTC[:, :,4]#heading down
# plt.matshow(TTC1)
# plt.show()
# TTC2=TTC1[x1:x2, y1:y2]
# # fromarray(TTC1.show()
# ax.add_patch(rect)
...
plt.suptitle('v='+str(np.float16(v_range[vind]))+ ' dxy=0.05 , dtheta=0.15 , uw=0.4, ua=1.1', x=0.1, y=.95, horizontalalignment='left', verticalalignment='bottom', fontsize = 15)
...
pos0=ax[1,1].matshow(obstacle_2d)
# ax[0,0].set_title('theta=-180')
ax[1,1].set_title('map')#(-180-90)%360
ax[1,1].title.set_position([.1,1.2])
plt.show()
# fig.tight_layout()
