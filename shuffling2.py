...
for data_file in data_files:
    with open(data_file, 'rb') as f:
        data = pickle.load(f)
        if 1 in data['labels'] and -1 in data['labels']:


            dictionary = {'start_pose': data['start_pose'][0].reshape(-1, 2, 1),  # (2, 1, 60)
                          # 'image': np.stack(imagez).reshape(-1, 224, 224, 3),
                        'image':  data['image'][0].reshape(-1, 224, 224, 3),
                          'waypointAction': data['waypointAction'].reshape(-1, 4, 1),
                          'labels': data['labels'].reshape(-1, 1)}
            listofdict.append(dictionary)

f = open("sample 1.pkl", "wb")
pickle.dump(listofdict, f)
dict_list = []
...
#         if num_ones != 0 :
#
#             num_ones_total.append(num_ones)
#
#             file_name+=1
...
from_each = 60
m = int((num_sample_each_file) / from_each)
...
for i in range(1, train_size):
    ...
    for k in range(0, m):
        ...
        if 1 in labelz[0] and -1 in labelz[0]:
            pass
        else:
            labelz = []
            break
        ...
        dictionary = {'start_pose': np.stack(posez).reshape(-1, 2, 1),  # (2, 1, 60)
                      # 'image': np.stack(imagez).reshape(-1, 224, 224, 3),
                    'image': listofdict[i]['image'][0].reshape(-1, 224, 224, 3),
    ...
    # if ((i+1) % 10) == 0:
        ...
        f = open("sample" + str(1) + str(int(i)) + '.pkl', "wb")
...
#     # if (i+1) % 10 == 0:
#         # dictionaries.append(dictionary)
#         f = open("sample" + str(2) + str(int(i) ) + '.pkl', "wb")
#         # 200
