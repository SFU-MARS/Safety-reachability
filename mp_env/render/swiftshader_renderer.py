...
class Shape():
    ...
    def __del__(self):
        # Todo: comment out __del__ because of an error braking the program
        commentOut = 1
        # scene = self.scene
...
class SwiftshaderRenderer():
    ...
    def get_salt_string(self):
        ...
        str_ = '{:s}-sz{:d}x{:d}-fov{:03d}x{:03d}-r{:04d}'.format(self.modality,
                                                                  self.height, self.width, int(self.fov_vertical),
                                                                  int(self.fov_horizontal),
    ...
    def init_renderer_egl(self, width, height):
        ...
        logging.debug("init_renderer_egl: egl_display: %s egl_surface: %s, egl_config: %s", egl_display, egl_surface,
                      egl_context)
    ...
    def render(self, take_screenshot=False, output_type=0):
        ...
        if take_screenshot:
            ...
            if self.modality == 'disparity':
                ...
                np_d_img = np_d_img[:, :, 2] * (255. * 255. / c) + np_d_img[:, :, 1] * (255. / c) + np_d_img[:, :,
                                                                                                    0] * (1. / c)
...
if __name__ == '__main__':
    ...
    _test_renderer('disparity', N=1600)# Copyright 2016 The TensorFlow Authors All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
